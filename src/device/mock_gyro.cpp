...
public:
    ...
    {
        if (imu->get_rotation() == PROS_ERR_F)
            return NAN;
...
public:
    ...
    double get_heading() const override
    {
        ...
        if (possiblyNegativeHeading < 0)
            return 360 + possiblyNegativeHeading;
        else
            return possiblyNegativeHeading;
    }
    ...
    bool calibrate()
    {
        ...
        if (doneCount == 0)
            return false;
        else if (doneCount == sources.size())
            return true;
        else
        {
