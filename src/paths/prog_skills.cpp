...
void prog_skills()
{
    ...
    chassis->moveToPoint(-65, -48, 2000, {.maxSpeed = 60});
    chassis->waitUntilDone();
    ...
    chassis->moveToPoint(0, 38, 3000, {.forwards = false, .minSpeed = 20, .earlyExitRange = 5});
    // chassis->moveToPoint(0, -68, 7000, {.maxSpeed = 90});
    chassis->moveToPose(0, 61.5, 0, 2200, {.lead = 0.3});
    tap();
    intake.set_state(mechanism::IntakeState::FIRST_HOOK);
    // pros::delay(600);
    chassis->waitUntilDone();
    ...
    */
    ...
    chassis->moveToPose(48, 0, 0, 4000, {.forwards = false});
    clamp.set_autoclamp(true);
    while (chassis->isInMotion() && !clamp.get_value())
    {
        pros::delay(20);
    ...
    intake.set_state(mechanism::IntakeState::HOOK);
    // first set of rings
    chassis->moveToPoint(26, -20.5, 3000);
    // under ladder
    chassis->moveToPoint(3, -3, 3000, {.maxSpeed = 80});
    pros::delay(500);
    intake.set_state(mechanism::IntakeState::FIRST_HOOK);
    ...
    chassis->moveToPoint(62.5, 36, 3000, {.forwards = false, .minSpeed = 20, .earlyExitRange = 5});
    chassis->moveToPose(62.5, 62, 5, 1500);
    doinker.extend();
    chassis->turnToHeading(20, 1000, {.direction = lemlib::AngularDirection::CCW_COUNTERCLOCKWISE});
    ...
    // alliance stakes
    chassis->moveToPoint(65, -48, 1500);
