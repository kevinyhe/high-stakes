...
void prog_skills()
{
    ...
    pros::delay(500);
    chassis->cancelMotion();
    chassis->setPose(chassis->getPose().x, -60, chassis->getPose().theta);
    pros::delay(200);
    ...
    // second set of rings
    intake.set_state(mechanism::IntakeState::HOOK);
    chassis->swingToPoint(-24, -48, lemlib::DriveSide::LEFT, 3000, {.minSpeed = 80, .earlyExitRange = 30});
    ...
    // second set of rings
    intake.set_state(mechanism::IntakeState::HOOK);
    chassis->swingToPoint(-24, 48, lemlib::DriveSide::RIGHT, 3000, {.minSpeed = 80, .earlyExitRange = 30});
    chassis->moveToPoint(-65, 48, 2500, {.maxSpeed = 60});
    chassis->waitUntilDone();
    chassis->setPose(-61.75, chassis->getPose().y, chassis->getPose().theta);
    pros::delay(200);
    ...
    // third mogo
    chassis->moveToPoint(48, 48, 4000, {.minSpeed = 12, .earlyExitRange = 4});
    arm.set_state(mechanism::ArmState::IDLE);
    pros::delay(1000);
    intake.set_state(mechanism::IntakeState::HOOK);
    ...
    chassis->waitUntilDone();
    chassis->setPose(61.75, chassis->getPose().y, chassis->getPose().theta);
    chassis->moveToPoint(46, 37, 3000, {.minSpeed = 20, .earlyExitRange = 8});

    chassis->moveToPoint(48, 0, 3000, {.forwards = false, .maxSpeed = 70});
    clamp.set_autoclamp(true);
    while (chassis->isInMotion() && !clamp.get_value())
    {
        pros::delay(20);
